...
function AvatarStreamFrameBase({ url, title, className = '' }: AvatarStreamFrameProps) {
  ...
  useEffect(() => {
    iframeUrlRef.current = normalizedUrl;
    setIsConnected(Boolean(normalizedUrl));
    setIsLoaded(false);
    setIsSlow(false);
  }, [normalizedUrl]);
  ...
  return (
    <div className={`relative h-full w-full overflow-hidden bg-slate-950 ${className}`}>
      ...
      {canConnect && isSlow && !isLoaded ? (
        <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between gap-3 rounded-xl border border-cyan-200/20 bg-slate-950/75 px-4 py-3 text-xs text-cyan-50 shadow-2xl backdrop-blur">
          <span>Waiting for the Unreal Pixel Streaming player to paint video.</span>
          <button
            type="button"
            onClick={() => {
              setIsLoaded(false);
            ...
            className="inline-flex shrink-0 items-center gap-2 rounded-lg bg-cyan-300 px-3 py-2 text-xs font-black text-slate-950"
          >
            <RotateCw size={14} />
            Retry
          </button>

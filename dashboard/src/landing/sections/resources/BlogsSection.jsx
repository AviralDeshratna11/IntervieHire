  ...
  return (
    <div ref={ref} style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
        <span style={{
          fontFamily: 'Outfit, sans-serif', fontSize: 'clamp(10px,1.6vw,11px)', fontWeight: 700,
          textTransform: 'uppercase', letterSpacing: '0.06em',
          padding: '4px 10px', borderRadius: 100,
          background: 'rgba(45,212,191,0.1)', color: T.gold, border: '1px solid rgba(45,212,191,0.15)'
        }}>{post.tag}</span>
        <span style={{ fontFamily: 'Outfit,sans-serif', fontSize: 'clamp(11px,1.6vw,12px)', color: T.muted }}>{post.readTime}</span>
      </div>
      <div style={{ fontFamily: 'Outfit,sans-serif', fontSize: 'clamp(11px,1.6vw,12px)', color: T.muted, marginBottom: 10, fontWeight: 500 }}>{post.date}</div>
      ...
      <a href="#" style={{
        fontFamily: 'Outfit,sans-serif', fontSize: 'clamp(12px,1.8vw,13px)', color: T.white, textDecoration: 'none',
...
export const BlogsSection = () => {
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      <section data-scroll id="blogs" style={{ background: T.bg, padding: 'clamp(100px,14vh,160px) clamp(16px,4vw,48px) clamp(60px,8vw,100px)', borderTop: '1px solid rgba(45,212,191,0.08)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: 8, marginBottom: 'clamp(32px,5vw,48px)' }}>
            {tags.map(tag => (
              <button key={tag} onClick={() => setActiveTag(tag)} style={{
                fontFamily: 'Outfit, sans-serif', fontSize: 'clamp(11px,1.8vw,12px)', fontWeight: 600,
                textTransform: 'uppercase', letterSpacing: '0.06em',
                padding: '8px 18px', borderRadius: 100,
                cursor: 'pointer', transition: 'all 0.3s ease',
                background: activeTag === tag ? T.gold : 'rgba(45,212,191,0.06)',
                color: activeTag === tag ? '#000' : T.muted,
                border: activeTag === tag ? 'none' : '1px solid rgba(45,212,191,0.15)'
              }}>
                {tag}
              </button>
            ))}
          ...
          {/* Blog grid */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(300px, 100%), 1fr))', gap: 'clamp(16px,2.5vw,24px)' }}>

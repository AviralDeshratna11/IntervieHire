...
// ==========================================
function updateSlidingPill(container: HTMLElement): void {
  if (!container) return;
  ...
  let pill = container.querySelector<HTMLElement>('.sliding-pill');
  if (!pill) {
...
function updateAllSlidingPills(): void {
...
function initSlidingPills(): void {
  const tracks = document.querySelectorAll('.sidebar-nav ul, .filter-options, .table-tabs, #team-status-tabs, .report-tabs, .jd-tabs, .sub-nav, .sourcing-mode-toggle');
